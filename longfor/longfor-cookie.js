...
[Script]
龙湖天街Cookie = type=http-request,pattern=^https?:\/\/gw2c-hw-open\.longfor\.com\/lmarketing-task-api-mvc-prod\/openapi\/task\/v1\/signature\/clock,script-path=https://raw.githubusercontent.com/MrNanko/scripts/main/longfor/longfor-cookie.js,script-update-interval=0,binary-body-mode=0,requires-body=1,max-size=0,timeout=1000
...
async function getCookie() {
  try {
    ...
    if (!header.cookie) {
      console.log('⚠️ 未找到 Cookie');
      return null;
    }

    const requiredHeaders = ['x-lf-dxrisk-token', 'x-lf-channel', 'token', 'x-lf-usertoken', 'x-lf-bu-code', 'x-lf-dxrisk-source'];
    const missingHeaders = requiredHeaders.filter(key => !header[key]);

    if (missingHeaders.length > 0) {
      console.log(`⚠️ 缺少必要的请求头: ${missingHeaders.join(', ')}`);
      return null;
    }
    ...
    const suffix = header.token;
    console.log(`✅ 获取到用户 ${suffix} 的 Cookie：${JSON.stringify(cookie)}`);
    return { suffix, cookie };
  } catch (e) {
