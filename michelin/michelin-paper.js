...
const $ = new Env('米其林答题Paper');

const config = {
    appName: 'michelin:paper',
    cookieTimeout: -1,
...
async function getCookie() {
    try {
        ...
        let query = getQueries($request?.url);
        if (!(query?.ulpUserPaperId && query?.paperCode)) throw new Error("参数缺少");
        $.setdata(query.ulpUserPaperId, "michelin_userPaper");
        $.setdata(query.paperCode, "michelin_banner");
        $.msg($.name, "获取数据成功！", `[paperId] ${query.ulpUserPaperId}\n[paperCode] ${query.paperCode}`);
    } catch (e) {
