...
const userList = (() => {
    if ($.isNode()) {
        const raw = process.env[config.appName] || process.env.sf || '[]';
        return $.toObj(raw, []);
    }
    return $.getjson(config.appName, []);
})();
...
async function getCookie() {
    ...
    if (!(userId && token.url && mobile)) {
        console.log('⚠️ 获取token失败，参数缺失');
        return null;
    }

    const cookie = { userId, token, userName: mobile };
    ...
    console.log(`✅ 获取到用户 ${desensitize(mobile)} 的 token: ${JSON.stringify(cookie)}`);
    return isUpdate === true ? { suffix: mobile, cookie } : null;
}
...
function updateUserData(list, newData, options = {}) {
    ...
    if (index !== -1) {
        // update
        if ($.toStr(list[index][valueColumn]) === $.toStr(value)) {
            return false;
        }
